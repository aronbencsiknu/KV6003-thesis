...
class CSNN(nn.Module):
    def __init__(self, batch_size, beta=0.5, spike_grad=surrogate.fast_sigmoid(slope=25)):
        ...
        self.maxpool = nn.MaxPool1d(3, stride=2)
        self.lif1 = snn.Leaky(beta=beta, spike_grad=spike_grad, learn_threshold=True)
        ...
        self.lif3 = snn.Leaky(beta=beta, spike_grad=spike_grad, learn_threshold=True)

        self.fc2 = nn.Linear(256, 2)
        self.lif4 = snn.Leaky(beta=beta, spike_grad=spike_grad, learn_threshold=True)
        self.lsm = nn.LogSoftmax(dim=1)
    ...
    def forward(self, x, num_steps, time_first=False):
        ...
        for step in range(num_steps):
            ...
            cur1 = self.maxpool(cur1)
            spk1, mem1 = self.lif1(cur1, mem1)

            cur2 = self.conv2(spk1)
            cur2 = self.adaptive_pool(cur2)
            spk2, mem2 = self.lif2(cur2, mem2)

            spk2 = torch.flatten(spk2, start_dim=1)
            cur3 = self.fc1(spk2)
            ...
            spk_rec.append(spk4.clone())
            mem_rec.append(mem4.clone())
